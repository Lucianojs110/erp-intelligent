    ...
    public function index()
    {
        ...
        for($i = 29; $i >= 0; $i--){
            ...
            if( !empty($sells_last_30_days[$date])){
                $values[$i] = $sells_last_30_days[$date];
            } else {
                $values[$i] = 0;
            }

            $sells_30_days[] = ['name' => date('j M Y', strtotime($date)), 'y' => floatval($values[$i])];
        }

        
        $json_sells_30_days = json_encode($sells_30_days);
        ...
        do {
            ...
            $labels[$i] = \Carbon::createFromFormat('m-Y', $month_year)
                            ->format('M-Y');
            $date = strtotime('+1 month', $date);

            if (!empty($sells_this_fy[$month_year])) {
                $values[$i] = $sells_this_fy[$month_year];
            } else {
                $values[$i] = 0;
            }
            $sells_per_year[] = ['name' => $labels[$i], 'y' => floatval($values[$i])];

        } while ($month_year != $last);

       
        
        $json_sells_per_year = json_encode($sells_per_year);
        ...
        return view('home.index', compact('date_filters',  'widgets', 'json_sells_30_days', 'json_sells_per_year'));
    }
